...
# crf = CRFBiRNNModel(icsup.tagset, icsup.vocab, lexicon)
crf = CRFBiRNNModel.load("my_crf.pkl")
...
# crf.train(corpus=icsup, loss=cross_entropy_loss, 
...
tagger_write_output(crf, icraw, Path("icraw_crf.output"))  # calls hmm.viterbi_tagging on each sentence
...
#     prob = math.exp(crf.log_prob(sentence, icraw))
...
# crf.train(corpus=icraw, loss=negative_log_likelihood,
